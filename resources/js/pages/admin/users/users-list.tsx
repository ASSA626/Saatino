...
export default function UsersList({users, users_count}: UsersListProps) {
    ...
    const usersTable = [
        ...
        { key: 'activity_status', label: 'فعالیت', render: (value: string) => (<StatusBadges status={value}/>) },
        { key: 'salary_status', label: 'تنخواه', render: (value: string) => (<StatusBadges status={value}/>) },
        { key: 'bimeh_status', label: 'بیمه', render: (value: string) => (<StatusBadges status={value}/>) },
        { key: 'car_status', label: 'خودرویی', render: (value: string) => (<StatusBadges status={value}/>) },
        { key: 'lunch_status', label: 'بن ناهار', render: (value: string) => (<StatusBadges status={value}/>) },
        {
            key: 'delete_action',
            label: 'حذف',
            action: {
                ...
                <div className="flex w-full flex-1 flex-col gap-8 px-5 sm:px-8 py-7 lg:py-12 xl:max-h-screen xl:overflow-y-scroll">
                    <div className="flex w-full flex-col gap-6">
                        ...
                        <TableInfo count={users_count} title="کاربران ثبت شده">
                            <button className="max-md:hidden text-blue-900 font-bold cursor-pointer" onClick={() => setAddUserState(true)}>
                                افزودن کاربر
                            </button>
                        </TableInfo>
