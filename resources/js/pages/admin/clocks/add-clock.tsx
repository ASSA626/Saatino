...
type AddClockProps = {
    addClockState: boolean;
    setAddClockState: (state: boolean) => void;
}
...
export default function AddClock({addClockState, setAddClockState}: AddClockProps) {
    ...
    const handleCreateClock = (e: FormEvent) => {
        e.preventDefault()
        post(route('admin.createClock'), {
            onSuccess: () => {
                reset();
                setWorklogs([]);
                setRemainingTime(0);
                setAddClockState(false);
            }
        })
    }

    return (
        <DialogLayout state={addClockState} setState={setAddClockState} title="ثبت ساعت جدید">
            <form onSubmit={handleCreateClock} className="max-md:mt-4">
